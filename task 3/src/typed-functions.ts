...
{
  type useOneNumber = (number: number) => number;
  type useTwoNumbers = (num1: number, num2: number) => number;
  type useStringVoid = (str: string) => void;

  const multiply: useTwoNumbers = (a, b) => a * b;
  
  const power: useTwoNumbers = function (base, pow) {
    return base ** pow;
  };
  
  const squareRoot: useOneNumber = (number) => number ** (1 / 2);
  
  const root: useTwoNumbers = function (base, nthRoot) {
    return base ** (1 / nthRoot);
  };
  
  const printBlueText: useStringVoid = (text) => console.log(`%c${text}`, 'color: #0000ee');
