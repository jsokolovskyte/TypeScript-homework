...
{
  const getLastMemberIndex = (ar: string[]): number => ar.lastIndexOf(ar[ar.length - 1]);

  console.log(getLastMemberIndex(words));
}
console.groupEnd();

console.group('2. Parašykite funkciją, kuri atspausdina kiekvieno masyvo elemento indeksus eilutėmis');
{
  function printArrayIndex(ar: string[]) {
    ar.forEach((_item: string, index: number): void => {
      console.log(index);
    });
  }
  printArrayIndex(words);
}
console.groupEnd();

console.group('3. Parašykite funkciją, kuri atspausdina kiekvieno masyvo elemento reikšmes eilutėmis');
{
  function printArrayItem(ar: string[]) {
    // eslint-disable-next-line no-unused-vars
    ar.forEach((item: string, _index: number): void => {
      console.log(item);
    });
  }
  printArrayItem(words);
}
console.groupEnd();
...
{
  function printArrayItemAndIndex(ar: string[]) {
    ar.forEach((item: string, index: number): void => {
      console.log([index], '=>', item);
    });
  }
  printArrayItemAndIndex(words);
}
console.groupEnd();
